#ifndef GIS_DATA_UTIL_H
#define GIS_DATA_UTIL_H
...
template <class T>
void getUnitVector(const T px2, const T px1, T &unit_vector, double &magnitude)
{
  auto vec = px2 - px1;
  magnitude = vec.norm();
  if (magnitude > 0)
  {
    unit_vector = vec.normalized();
  }
  else
  {
    unit_vector = vec;
...
template <class T>
void getUnitVector(const T px2, T &unit_vector, double &magnitude)
{
  auto vec = px2;
  magnitude = vec.norm();
  if (magnitude > 0)
...
#define getUnitVector2d getUnitVector<Vec2f>
#define getUnitVector3d getUnitVector<Vec3f>
